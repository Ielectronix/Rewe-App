...
def main(page: ft.Page):
    ...
    try:
        ...
        # SICHERE FUNKTION ZUM TEILEN (Verhindert Fehler in der Schleife)
        def get_teilen_handler(ziel_pfad, modus="postausgang"):
            async def on_teilen_click(e):
                # 1. Sofort im Speicher abhaken
                markiere_als_gesendet(ziel_pfad)
                
                # 2. Sofort die Seite neu laden (Macht den Haken sichtbar!)
                if modus == "postausgang":
                    zeige_postausgang()
                else:
                    zeige_archiv()
                    
                # 3. Erst ganz am Ende das Teilen-Menü von Android öffnen
                if share_obj:
        ...
        def zeige_postausgang():
            ...
            # WICHTIG: Holt den frischesten Stand!
