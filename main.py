...
def main(page: ft.Page):
    ...
    try:
        ...
        def zeige_startbildschirm():
            ...
            ansicht.controls.extend([ft.Container(height=50), ft.Row([header], alignment=ft.MainAxisAlignment.CENTER), ft.Container(height=40), ft.Column([v_in, z_in]), ft.Container(height=40), ft.Row([ft.ElevatedButton("Neuen Tag starten", on_click=start_klick, bgcolor="red", color="white", height=60, expand=True)])])
            page.update()
        ...
        def zeige_dashboard():
            ...
            if not maerkte:
                ...
                for index, markt in enumerate(maerkte):
                    ...
                    # HIER: Mülleimer-Button stark verschmälert (width=45 und padding=0) für mehr Platz beim Text
        ...
        def zeige_maske(markt_index):
            ...
            bem_in = ft.TextField(label="Eigene Bemerkung", value=aktuelle_daten.get("bemerkung"), color="yellow", text_style=stil_tf_gelb_10, label_style=stil_label_weiss, border_color="white", text_size=10, visible=True)
            vor_dd = ft.Dropdown(label="Vorlage wählen ▼", color="yellow", border_color="white", text_style=stil_tf_gelb_10, label_style=stil_label_rot_fett, options=[ft.dropdown.Option("keine Fertiggerichte"), ft.dropdown.Option("keine Convinience heute"), ft.dropdown.Option("keine HFM heute"), ft.dropdown.Option("keine Convinience generell")], visible=False)
            ...
            bemerkungs_container = ft.Column([bem_in, vor_dd])
