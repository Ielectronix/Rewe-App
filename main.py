...
def main(page: ft.Page):
    ...
    try:
        ...
        def zeige_maske(markt_index):
            ...
            auft_in = ft.TextField(
                label="Auftragsnummer", 
                hint_text="Etikettennummer: XX-XXXXXX", 
                hint_style=ft.TextStyle(color="white54", size=10), 
                value=aktuelle_daten.get("auftragsnummer"), 
                color="yellow", 
                text_style=stil_tf_gelb_12, 
                label_style=stil_label_weiss, 
                border_color="white", 
                text_size=12
            )
            ...
            # ==========================================
            # TRINKWASSER Felder & WÄCHTER (Ohne LIMS-Begriff)
            # ==========================================
            ...
            def pruefe_lims_warnung(e=None):
                ...
                if not braucht_warnung: 
                    tw_lims_override_cb.value = False 
                page.update()

            def format_zeit(e):
                val = e.control.value or ""
                zahlen = "".join([c for c in val if c.isdigit()])
                zahlen = zahlen[:4]
                if len(zahlen) >= 3:
                    neu_wert = zahlen[:2] + ":" + zahlen[2:]
                else:
                    neu_wert = zahlen
            ...
            # ==========================================
            # Gemeinsame Buttons & Fehler-Logik
            # ==========================================
            ...
            def check_pflichtfelder():
                ...
                hat_daten = bool((tw_zeit_in.value or "").strip() or (tw_temp_in.value or "").strip())
                if hat_daten and not tw_kalt_cb.value:
                    if not tw_lims_override_cb.value:
                        wechsle_zu_tw()
                        # Fehlermeldung ebenfalls angepasst
            ...
            def pdf_speichern(e):
                ...
                try:
                    ...
                    f_map = {"tf_0000_00_ZS-001870": adr_in.value, "tf_0000_00_ZS-1408": nr_in.value, "tf_0000_00_ZS-002000": auft_in.value, "cal_templateLaborderprobenahmeDatum": heute_str, "dd_0000_00_ZS-002314": name_in.value, "dd_0000_00_ZS-1566": ag_dd.value, "dd_0000_00_ZS-002315": typ_dd.value, "dd_0000_00_ZS-001796": text_fuer_pdf}
            ...
            save_btn = ft.ElevatedButton("Speichern", bgcolor="blue", color="white", height=35, expand=True)
            def save_klick(e):
                if not check_pflichtfelder(): return 
                text_fuer_json = bem_in.value if bem_in.visible else vor_dd.value
                
                d = {
                    "adresse": adr_in.value, "marktnummer": nr_in.value, "auftragsnummer": auft_in.value, "mitarbeiter_name": name_in.value, "auftraggeber": ag_dd.value, "typ_probenahme": typ_dd.value, "bemerkung": text_fuer_json,
            ...
            btn_pdf = ft.ElevatedButton("Bericht erstellen", on_click=pdf_speichern, bgcolor="blue", color="white", height=35, expand=True)
            ...
            ansicht.controls.extend([
                ...
                fehler_text, ft.Row([save_btn, btn_touren, btn_pdf], spacing=5)
            ])
            page.update()
