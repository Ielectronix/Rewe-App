...
def main(page: ft.Page):
    ...
    try:
        ...
        # SICHERE FUNKTION ZUM TEILEN
        # Der Haken wird erst gesetzt, wenn Android aus dem Teilen-Dialog zurückkommt.
        def get_teilen_handler(ziel_pfad, modus="postausgang"):
            async def on_teilen_click(e):
                try:
                    if share_obj:
                        await share_obj.share_files([ft.ShareFile.from_path(ziel_pfad)], text="REWE Bericht")
        ...
        def zeige_postausgang():
                    ...
                    for f in os.listdir(ordner):
                        if f.lower().endswith(".pdf"):
                            ...
                            ansicht.controls.append(ft.Container(bgcolor="#002200", padding=10, border_radius=15, content=ft.Row([
                                ft.Text(f, color=text_farbe, size=13, expand=True, max_lines=2, overflow=ft.TextOverflow.ELLIPSIS),
            ...
            if not pdfs_gefunden:
                ansicht.controls.append(ft.Text("Keine Berichte zum Senden.", color="white54", text_align="center"))
            page.add(ft.SafeArea(ansicht))
            page.update()
        ...
        def zeige_archiv():
            ...
            if not pdfs_gefunden:
                ansicht.controls.append(ft.Text("Keine Berichte im Archiv.", color="white54", text_align="center"))
            page.add(ft.SafeArea(ansicht))
            page.update()
