...
def zeige_maske_ui(page: ft.Page, ansicht: ft.Column, nav_leiste, zeige_dashboard, zeige_fehler, markt_index):
    try:
        ...
        if markt_index is None:
            aktuelle_daten = {"datum": heute_str, "mitarbeiter_name": f"{v} {z}".strip(), "auftraggeber": "03509 - REWE Hackfleischmonitoring", "typ_probenahme": "Standard"}
            titel = "Neue Tour"
        else:
        ...
        def tf(label, val, hint="", w=320, oc=None, ob=None):
            return ft.TextField(label=label, value=val, hint_text=hint, hint_style=ft.TextStyle(color="white54", size=12), color="yellow", text_style=ft.TextStyle(size=12, color="yellow"), label_style=ft.TextStyle(color="white"), border_color="white", content_padding=10, width=w, on_change=oc, on_blur=ob)

        def cb(label, val, oc=None, bold=False):
            return ft.Checkbox(label=label, value=val, on_change=oc, label_style=ft.TextStyle(color="white", size=16 if bold else 12, weight="bold" if bold else "normal"), fill_color="yellow", check_color="black")

        def combo(label, val, opts, w=320, oc=None):
            def intern_oc(e):
                if oc: oc(e)
            c = ft.TextField(label=label, value=val, color="yellow", text_style=ft.TextStyle(size=12, color="yellow"), label_style=ft.TextStyle(color="white"), border_color="white", dense=True, content_padding=10, width=w, on_change=intern_oc)
        ...
        # DAS NEUE EDLE FARBRAND-DESIGN FÜR DIE AKTIONEN UNTEN!
        def action_btn(text, oc, farbe):
            return ft.ElevatedButton(
                content=ft.Text(text, size=14, weight="bold"),
                on_click=oc, bgcolor="#1a1a1a", color=farbe,
                style=ft.ButtonStyle(
                    shape=ft.RoundedRectangleBorder(radius=25), 
                    padding=ft.padding.symmetric(horizontal=20, vertical=15),
                    side=ft.BorderSide(width=2, color=farbe) # Neon-Farbrand!
                )
            )
        ...
        def pruefe_lims_warnung(e=None): pass
        ...
        # --- 2. TRINKWASSER ---
        tw_kalt_cb = cb("Trinkwasser kalt", aktuelle_daten.get("tw_kalt", False), pruefe_lims_warnung, True)
        tw_zeit_in, tw_temp_in, tw_tempkonst_in = tf("Probenahmezeit", aktuelle_daten.get("tw_zeit"), oc=format_zeit), tf("Temp Probenahme", aktuelle_daten.get("tw_temp"), ob=format_temp), tf("Temp Konstante", aktuelle_daten.get("tw_tempkonst"), ob=format_temp)
        tw_desinf_dd, tw_zapf_dd = combo("Desinfektion", aktuelle_daten.get("tw_desinf", "Abflammen"), ["Abflammen", "Sprühdesinfektion", "ohne Desinfektion"]), combo("Zapfstelle", aktuelle_daten.get("tw_zapf", "Spülbecken"), ["Spülbecken", "Handwaschbecken"])
        tw_zapf_sonst_dd, tw_inaktiv_dd = combo("Sonstiges Zapfstelle", aktuelle_daten.get("tw_zapf_sonst", ""), ["Schlaucharmatur", "Schlauchbrause", "Schlauch mit Brause"]), combo("Inaktivierung", aktuelle_daten.get("tw_inaktiv", "Na-Thiosulfat"), ["Na-Thiosulfat"])
        tw_kurz1_dd, tw_kurz2_dd = combo("Farbe", aktuelle_daten.get("tw_kurz1", "1 - nicht wahrnehmbar"), ["1 - nicht wahrnehmbar", "2 - wahrnehmbar", "3 - deutlich wahrnehmbar"]), combo("Trübung", aktuelle_daten.get("tw_kurz2", "1 - nicht wahrnehmbar"), ["1 - nicht wahrnehmbar", "2 - wahrnehmbar", "3 - deutlich wahrnehmbar"])
        tw_kurz3_dd, tw_kurz4_dd = combo("Bodensatz", aktuelle_daten.get("tw_kurz3", "1 - nicht wahrnehmbar"), ["1 - nicht wahrnehmbar", "2 - wahrnehmbar", "3 - deutlich wahrnehmbar"]), combo("Geruch", aktuelle_daten.get("tw_kurz4", "1 - nicht wahrnehmbar"), ["1 - nicht wahrnehmbar", "2 - wahrnehmbar", "3 - deutlich wahrnehmbar"])
        tw_zweck_dd, tw_verpackung_dd = combo("Zweck", aktuelle_daten.get("tw_zweck", "Zweck B"), ["Zweck A", "Zweck B", "Zweck C"]), combo("Verpackung", aktuelle_daten.get("tw_verpackung", "500ml Kunststoff-Flasche mit Natriumthiosulfat"), ["500ml Kunststoff-Flasche mit Natriumthiosulfat"])
        tw_entnahmeort_dd, tw_bemerkung_dd = combo("Entnahmeort", aktuelle_daten.get("tw_entnahmeort", "Metzgerei"), ort_opts), combo("TW Bemerkung", aktuelle_daten.get("tw_bemerkung_2", ""), ["", "Keine Besonderheiten"])
        ...
        tw_auff_sonstiges_in, tw_inhalt_in = tf("Auffälligkeiten (Sonstiges)", aktuelle_daten.get("tw_auff_sonst_text")), tf("Inhalt", aktuelle_daten.get("tw_inhalt", "ca. 500 ml"))

        # --- 3. SCHERBENEIS ---
        se_kalt_cb = cb("Scherbeneis Eigenkontrolle", aktuelle_daten.get("se_kalt", False), pruefe_lims_warnung, True)
        se_zeit_in, se_zapf_dd = tf("Probenahmezeit", aktuelle_daten.get("se_zeit"), oc=format_zeit), combo("Zapfstelle (Eis)", aktuelle_daten.get("se_zapf", "Eismaschine"), ["Eismaschine"])
        ...
        se_tech_sonst_in, se_desinf_dd = tf("Sonstiges (Technik)", aktuelle_daten.get("se_tech_sonst")), combo("Art der Desinfektion", aktuelle_daten.get("se_desinf", "ohne Desinfektion"), ["Abflammen", "Sprühdesinfektion", "ohne Desinfektion"])
        ...
        se_inhalt_in, se_verpackung_dd = tf("Inhalt", aktuelle_daten.get("se_inhalt", "ca. 1000ml")), combo("Verpackung", aktuelle_daten.get("se_verpackung", "steriler Probenbeutel"), ["steriler Probenbeutel"])
        se_entnahmeort_dd, se_temp_in = combo("Entnahmeort", aktuelle_daten.get("se_entnahmeort", "Fischabteilung-Eismaschine"), ["Fischabteilung-Eismaschine", "Metzgerei", "Produktionsraum"]), tf("Probenahmetemperatur", aktuelle_daten.get("se_temp"), ob=format_temp)
        se_bemerkung_dd = combo("Bemerkungen", aktuelle_daten.get("se_bemerkung", ""), ["", "Keine Besonderheiten"])

        se_okz_cb = cb("Abklatschproben Scherbeneis", aktuelle_daten.get("se_abklatsch_cb", False), pruefe_lims_warnung, True)
        ...
        for i in range(1, 4):
            idx = f"{i:02d}"
            se_okz_controls[idx] = {"status": combo("Status", aktuelle_daten.get(f"0003_status_{idx}", "R+D"), ["R+D", "R", "P", "-"], 100), "objekt": combo("Objekt", aktuelle_daten.get(f"0003_objekt_{idx}", se_okz_def[i]), se_okz_opts, 200), "ort": combo("Probenahmeort", aktuelle_daten.get(f"0003_ort_{idx}", ""), ["Fischabteilung", "Metzgerei", "Produktionsbereich", "Kühlraum"]), "abklatsch": cb("Abklatsch", aktuelle_daten.get(f"0003_abklatsch_{idx}", True)), "tupfer": cb("Tupfer", aktuelle_daten.get(f"0003_tupfer_{idx}", True))}

        # --- 4. HFM ---
        hfm_hack_cb = cb("Hackfleisch gemischt", aktuelle_daten.get("hfm_hack_cb", False), pruefe_lims_warnung, True)
        ...
        hfm_hack_mhd_s_tag_dd, hfm_hack_mhd_s_mon_dd, hfm_hack_mhd_s_jahr_dd = combo("Tag", t, tage_opts, 90, pruefe_lims_warnung), combo("Mon", m, mon_opts, 90), combo("Jahr", j, jahr_opts, 120)
        t, m, j = parse_datum(aktuelle_daten.get("hfm_hack_mhd_rind", ""))
        hfm_hack_mhd_r_tag_dd, hfm_hack_mhd_r_mon_dd, hfm_hack_mhd_r_jahr_dd = combo("Tag", t, tage_opts, 90, pruefe_lims_warnung), combo("Mon", m, mon_opts, 90), combo("Jahr", j, jahr_opts, 120)
        hfm_hack_inhalt_in = tf("Inhalt", aktuelle_daten.get("hfm_hack_inhalt", "jeweils ca. 200 g"), "Grammzahl", ob=format_gramm)
        hfm_hack_verpackung_dd = combo("Verpackung", aktuelle_daten.get("hfm_hack_verpackung", "steriler Probenbeutel"), verp_opts)
        hfm_hack_lief_schwein_in, hfm_hack_lief_rind_in = tf("Lieferant (Schwein)", aktuelle_daten.get("hfm_hack_lief_schwein", ""), oc=pruefe_lims_warnung), tf("Lieferant (Rind)", aktuelle_daten.get("hfm_hack_lief_rind", ""), oc=pruefe_lims_warnung)
        hfm_hack_charge_schwein_dd, hfm_hack_charge_rind_dd = combo("Charge Schwein", aktuelle_daten.get("hfm_hack_charge_schwein", ""), c_opts_s, oc=pruefe_lims_warnung), combo("Charge Rind", aktuelle_daten.get("hfm_hack_charge_rind", ""), c_opts_r, oc=pruefe_lims_warnung)
        hfm_hack_temp_in, hfm_hack_bemerkung_dd = tf("Probenahmetemperatur", aktuelle_daten.get("hfm_hack_temp", ""), "<+7°C", ob=format_temp), combo("Bemerkungen", aktuelle_daten.get("hfm_hack_bemerkung", ""), ["", "Keine Besonderheiten"])

        hfm_mett_cb = cb("Gewürztes Schweinemett", aktuelle_daten.get("hfm_mett_cb", False), pruefe_lims_warnung, True)
        ...
        hfm_mett_mhd_tag_dd, hfm_mett_mhd_mon_dd, hfm_mett_mhd_jahr_dd = combo("Tag", t, tage_opts, 90, pruefe_lims_warnung), combo("Mon", m, mon_opts, 90), combo("Jahr", j, jahr_opts, 120)
        hfm_mett_inhalt_in, hfm_mett_verpackung_dd = tf("Inhalt", aktuelle_daten.get("hfm_mett_inhalt", "ca. 200 g"), "Grammzahl", ob=format_gramm), combo("Verpackung", aktuelle_daten.get("hfm_mett_verpackung", "steriler Probenbeutel"), verp_opts)
        hfm_mett_lief_in, hfm_mett_charge_dd = tf("Lieferant Rohware", aktuelle_daten.get("hfm_mett_lief", ""), oc=pruefe_lims_warnung), combo("Charge Rohware", aktuelle_daten.get("hfm_mett_charge", ""), c_opts_s, oc=pruefe_lims_warnung)
        hfm_mett_temp_in, hfm_mett_bemerkung_dd = tf("Probenahmetemperatur", aktuelle_daten.get("hfm_mett_temp", ""), "<+7°C", ob=format_temp), combo("Bemerkungen", aktuelle_daten.get("hfm_mett_bemerkung", ""), ["", "Keine Besonderheiten"])

        hfm_fzs_cb = cb("Fleischzubereitung Schwein", aktuelle_daten.get("hfm_fzs_cb", False), pruefe_lims_warnung, True)
        hfm_fzs_entnahmeort_dd = combo("Entnahmeort", aktuelle_daten.get("hfm_fzs_entnahmeort", "Kühlraum"), ort_opts)
        hfm_fzs_produkt_in, hfm_fzs_marinade_in = tf("Produkt", aktuelle_daten.get("hfm_fzs_produkt", ""), "z. B. Nacken", oc=pruefe_lims_warnung), tf("Marinade", aktuelle_daten.get("hfm_fzs_marinade", ""))
        ...
        hfm_fzs_mhd_tag_dd, hfm_fzs_mhd_mon_dd, hfm_fzs_mhd_jahr_dd = combo("Tag", t, tage_opts, 90, pruefe_lims_warnung), combo("Mon", m, mon_opts, 90), combo("Jahr", j, jahr_opts, 120)
        hfm_fzs_inhalt_in, hfm_fzs_verpackung_dd = tf("Inhalt", aktuelle_daten.get("hfm_fzs_inhalt", "ca. 200 g"), ob=format_gramm), combo("Verpackung", aktuelle_daten.get("hfm_fzs_verpackung", "steriler Probenbeutel"), verp_opts)
        hfm_fzs_lief_in, hfm_fzs_charge_dd = tf("Lieferant Rohware", aktuelle_daten.get("hfm_fzs_lief", ""), oc=pruefe_lims_warnung), combo("Charge Rohware", aktuelle_daten.get("hfm_fzs_charge", ""), c_opts_s, oc=pruefe_lims_warnung)
        hfm_fzs_temp_in, hfm_fzs_bemerkung_dd = tf("Probenahmetemperatur", aktuelle_daten.get("hfm_fzs_temp", ""), "<+7°C", ob=format_temp), combo("Bemerkungen", aktuelle_daten.get("hfm_fzs_bemerkung", ""), ["", "Keine Besonderheiten"])

        hfm_fzg_cb = cb("Fleischzubereitung Geflügel", aktuelle_daten.get("hfm_fzg_cb", False), pruefe_lims_warnung, True)
        hfm_fzg_entnahmeort_dd = combo("Entnahmeort", aktuelle_daten.get("hfm_fzg_entnahmeort", "Kühlraum"), ort_opts)
        hfm_fzg_produkt_in, hfm_fzg_marinade_in = tf("Produkt", aktuelle_daten.get("hfm_fzg_produkt", ""), "z. B. Brust", oc=pruefe_lims_warnung), tf("Marinade", aktuelle_daten.get("hfm_fzg_marinade", ""))
        ...
        hfm_fzg_mhd_tag_dd, hfm_fzg_mhd_mon_dd, hfm_fzg_mhd_jahr_dd = combo("Tag", t, tage_opts, 90, pruefe_lims_warnung), combo("Mon", m, mon_opts, 90), combo("Jahr", j, jahr_opts, 120)
        hfm_fzg_inhalt_in, hfm_fzg_verpackung_dd = tf("Inhalt", aktuelle_daten.get("hfm_fzg_inhalt", "ca. 200 g"), ob=format_gramm), combo("Verpackung", aktuelle_daten.get("hfm_fzg_verpackung", "steriler Probenbeutel"), verp_opts)
        hfm_fzg_lief_in, hfm_fzg_charge_dd = tf("Lieferant Rohware", aktuelle_daten.get("hfm_fzg_lief", ""), oc=pruefe_lims_warnung), combo("Charge Rohware", aktuelle_daten.get("hfm_fzg_charge", ""), c_opts_g, oc=pruefe_lims_warnung)
        hfm_fzg_temp_in, hfm_fzg_bemerkung_dd = tf("Probenahmetemperatur", aktuelle_daten.get("hfm_fzg_temp", ""), "<+4°C", ob=format_temp), combo("Bemerkungen", aktuelle_daten.get("hfm_fzg_bemerkung", ""), ["", "Keine Besonderheiten"])

        hfm_bio_cb = cb("Bio-Hackfleisch", aktuelle_daten.get("hfm_bio_cb", False), pruefe_lims_warnung, True)
        ...
        hfm_bio_mhd_s_tag_dd, hfm_bio_mhd_s_mon_dd, hfm_bio_mhd_s_jahr_dd = combo("Tag", t, tage_opts, 90, pruefe_lims_warnung), combo("Mon", m, mon_opts, 90), combo("Jahr", j, jahr_opts, 120)
        t, m, j = parse_datum(aktuelle_daten.get("hfm_bio_mhd_rind", ""))
        hfm_bio_mhd_r_tag_dd, hfm_bio_mhd_r_mon_dd, hfm_bio_mhd_r_jahr_dd = combo("Tag", t, tage_opts, 90, pruefe_lims_warnung), combo("Mon", m, mon_opts, 90), combo("Jahr", j, jahr_opts, 120)
        hfm_bio_inhalt_in, hfm_bio_verpackung_dd = tf("Inhalt", aktuelle_daten.get("hfm_bio_inhalt", "jeweils ca. 200 g"), ob=format_gramm), combo("Verpackung", aktuelle_daten.get("hfm_bio_verpackung", "steriler Probenbecher"), verp_opts)
        hfm_bio_lief_schwein_in, hfm_bio_lief_rind_in = tf("Lieferant (Schwein)", aktuelle_daten.get("hfm_bio_lief_schwein", ""), oc=pruefe_lims_warnung), tf("Lieferant (Rind)", aktuelle_daten.get("hfm_bio_lief_rind", ""), oc=pruefe_lims_warnung)
        hfm_bio_charge_schwein_dd, hfm_bio_charge_rind_dd = combo("Charge Schwein", aktuelle_daten.get("hfm_bio_charge_schwein", ""), c_opts_s, oc=pruefe_lims_warnung), combo("Charge Rind", aktuelle_daten.get("hfm_bio_charge_rind", ""), c_opts_r, oc=pruefe_lims_warnung)
        hfm_bio_temp_in, hfm_bio_bemerkung_dd = tf("Probenahmetemperatur", aktuelle_daten.get("hfm_bio_temp", ""), "<+7°C", ob=format_temp), combo("Bemerkungen", aktuelle_daten.get("hfm_bio_bemerkung", ""), ["", "Keine Besonderheiten"])

        hfm_okz_cb = cb("Abklatschproben HFM", aktuelle_daten.get("hfm_abklatsch_cb", False), pruefe_lims_warnung, True)
        ...
        for i in range(1, 11):
            idx = f"{i:02d}"
            okz_controls[idx] = {"status": combo("Status", aktuelle_daten.get(f"0010_status_{idx}", "R+D"), ["R+D", "R", "P", "-"], 100), "objekt": combo("Objekt", aktuelle_daten.get(f"0010_objekt_{idx}", okz_def[i]["o"]), okz_obj_opts, 200), "ort": combo("Probenahmeort", aktuelle_daten.get(f"0010_ort_{idx}", ""), ["Kühlraum", "Produktionsbereich", "Theke"]), "abklatsch": cb("Abklatsch", aktuelle_daten.get(f"0010_abklatsch_{idx}", okz_def[i]["a"])), "tupfer": cb("Tupfer", aktuelle_daten.get(f"0010_tupfer_{idx}", okz_def[i]["t"]))}

        # --- 5. CONVENIENCE ---
        og_cb = cb("Obst-/Gemüse Convenience", aktuelle_daten.get("og_cb", False), pruefe_lims_warnung, True)
        og_controls = {}
        for i in range(1, 6):
            idx = f"{i:02d}"
            ...
            og_controls[i] = {
                "name": tf(f"Name Teilprobe {i}", aktuelle_daten.get(f"og_name_{idx}", ""), oc=pruefe_lims_warnung),
                ...
                "v_t": combo("Tag", vt, tage_opts, 90, oc=pruefe_lims_warnung), "v_m": combo("Mon", vm, mon_opts, 90), "v_j": combo("Jahr", vj, jahr_opts, 120),
        ...
        og_okz_cb = cb("Abklatschproben Convenience", aktuelle_daten.get("og_abklatsch_cb", False), pruefe_lims_warnung, True)
        ...
        def switch_tab(tab_id):
            ...
            if tab_id == "stamm":
                ...
                haupt_bereich.controls.extend([ft.Text("Trinkwasser-Check", size=24, weight="bold", color="white"), tw_kalt_cb, tw_zeit_in, tw_temp_in, tw_tempkonst_in, ft.Divider(color="white24"), ft.Text("Probenahme / Zapfstelle:", color="white", weight="bold"), tw_entnahmeort_dd, tw_zapf_dd, tw_zapf_sonst_dd, tw_desinf_dd, ft.Row([cb_pn, cb_zwei, cb_sensor, cb_knie], wrap=True, alignment=ft.MainAxisAlignment.CENTER), ft.Row([cb_ein, cb_ein_g, cb_eck], wrap=True, alignment=ft.MainAxisAlignment.CENTER), ft.Divider(color="white24"), ft.Text("Sensorik & Analytik:", color="white", weight="bold"), tw_inaktiv_dd, tw_kurz1_dd, tw_kurz2_dd, tw_kurz3_dd, tw_kurz4_dd, ft.Divider(color="white24"), ft.Text("Auffälligkeiten:", color="white", weight="bold"), ft.Row([cb_auff_ja, cb_auff_nein], wrap=True, alignment=ft.MainAxisAlignment.CENTER), cb_auff_perl, cb_auff_verkalk, cb_auff_verbrueh, cb_auff_durchlauf, cb_auff_unterbau, cb_auff_eck_zu, cb_auff_nichtmoeglich, cb_auff_dusche, cb_auff_handbrause, cb_auff_sonst, tw_auff_sonstiges_in, ft.Divider(color="white24"), tw_zweck_dd, tw_inhalt_in, tw_verpackung_dd, tw_bemerkung_dd])
            elif tab_id == "se":
                sub_nav = ft.Row(wrap=True, alignment=ft.MainAxisAlignment.CENTER)
                sub_cont = ft.Column(horizontal_alignment=ft.CrossAxisAlignment.CENTER)
                ...
                def sw_se(sub):
                    ...
                    if sub == "eis": sub_cont.controls.extend([se_kalt_cb, se_zeit_in, se_zapf_dd, ft.Text("Technik:", color="white", weight="bold"), ft.Row([se_cb_eiswanne, se_cb_fallprobe], wrap=True, alignment=ft.MainAxisAlignment.CENTER), se_tech_sonst_in, se_desinf_dd, ft.Text("Auffälligkeiten:", color="white", weight="bold"), se_cb_ozon, se_auff_sonst_in, se_inhalt_in, se_verpackung_dd, se_entnahmeort_dd, se_temp_in, se_bemerkung_dd])
                    elif sub == "okz":
                        ...
                        for i in range(1, 4):
                            ...
                            sub_cont.controls.extend([ft.Text(f"Probe {i}", color="yellow", weight="bold"), ft.Row([c["status"], c["objekt"]], wrap=True, alignment=ft.MainAxisAlignment.CENTER), c["ort"], ft.Row([c["abklatsch"], c["tupfer"]], wrap=True, alignment=ft.MainAxisAlignment.CENTER), ft.Divider(color="white24")])
                ...
                def sw_hfm(sub):
                    ...
                    if sub == "hack": sub_cont.controls.extend([hfm_hack_cb, hfm_hack_entnahmeort_dd, ft.Text("Herstellungsdatum:", color="white"), ft.Row([hfm_hack_herst_tag_dd, hfm_hack_herst_mon_dd, hfm_hack_herst_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_hack_inhalt_in, hfm_hack_verpackung_dd, hfm_hack_lief_schwein_in, hfm_hack_lief_rind_in, ft.Text("MHD (Schwein):", color="yellow"), ft.Row([hfm_hack_mhd_s_tag_dd, hfm_hack_mhd_s_mon_dd, hfm_hack_mhd_s_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), ft.Text("MHD (Rind):", color="yellow"), ft.Row([hfm_hack_mhd_r_tag_dd, hfm_hack_mhd_r_mon_dd, hfm_hack_mhd_r_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_hack_charge_schwein_dd, hfm_hack_charge_rind_dd, hfm_hack_temp_in, hfm_hack_bemerkung_dd])
                    elif sub == "mett": sub_cont.controls.extend([hfm_mett_cb, hfm_mett_entnahmeort_dd, ft.Text("Herstellungsdatum:", color="white"), ft.Row([hfm_mett_herst_tag_dd, hfm_mett_herst_mon_dd, hfm_mett_herst_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_mett_inhalt_in, hfm_mett_verpackung_dd, hfm_mett_lief_in, ft.Text("MHD:", color="white"), ft.Row([hfm_mett_mhd_tag_dd, hfm_mett_mhd_mon_dd, hfm_mett_mhd_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_mett_charge_dd, hfm_mett_temp_in, hfm_mett_bemerkung_dd])
                    elif sub == "fzs": sub_cont.controls.extend([hfm_fzs_cb, hfm_fzs_entnahmeort_dd, hfm_fzs_produkt_in, hfm_fzs_marinade_in, ft.Text("Herstellungsdatum:", color="white"), ft.Row([hfm_fzs_herst_tag_dd, hfm_fzs_herst_mon_dd, hfm_fzs_herst_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_fzs_inhalt_in, hfm_fzs_verpackung_dd, hfm_fzs_lief_in, ft.Text("MHD:", color="white"), ft.Row([hfm_fzs_mhd_tag_dd, hfm_fzs_mhd_mon_dd, hfm_fzs_mhd_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_fzs_charge_dd, hfm_fzs_temp_in, hfm_fzs_bemerkung_dd])
                    elif sub == "fzg": sub_cont.controls.extend([hfm_fzg_cb, hfm_fzg_entnahmeort_dd, hfm_fzg_produkt_in, hfm_fzg_marinade_in, ft.Text("Herstellungsdatum:", color="white"), ft.Row([hfm_fzg_herst_tag_dd, hfm_fzg_herst_mon_dd, hfm_fzg_herst_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_fzg_inhalt_in, hfm_fzg_verpackung_dd, hfm_fzg_lief_in, ft.Text("MHD:", color="white"), ft.Row([hfm_fzg_mhd_tag_dd, hfm_fzg_mhd_mon_dd, hfm_fzg_mhd_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_fzg_charge_dd, hfm_fzg_temp_in, hfm_fzg_bemerkung_dd])
                    elif sub == "bio": sub_cont.controls.extend([hfm_bio_cb, hfm_bio_entnahmeort_dd, ft.Text("Herstellungsdatum:", color="white"), ft.Row([hfm_bio_herst_tag_dd, hfm_bio_herst_mon_dd, hfm_bio_herst_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_bio_inhalt_in, hfm_bio_verpackung_dd, hfm_bio_lief_schwein_in, hfm_bio_lief_rind_in, ft.Text("MHD (Schwein):", color="yellow"), ft.Row([hfm_bio_mhd_s_tag_dd, hfm_bio_mhd_s_mon_dd, hfm_bio_mhd_s_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), ft.Text("MHD (Rind):", color="yellow"), ft.Row([hfm_bio_mhd_r_tag_dd, hfm_bio_mhd_r_mon_dd, hfm_bio_mhd_r_jahr_dd], wrap=True, alignment=ft.MainAxisAlignment.CENTER), hfm_bio_charge_schwein_dd, hfm_bio_charge_rind_dd, hfm_bio_temp_in, hfm_bio_bemerkung_dd])
                    elif sub == "okz":
                ...
                def sw_og(sub):
                    ...
                    if sub == "teil":
                        ...
                        for i in range(1, 6):
                            c = og_controls[i]
                            sub_cont.controls.extend([ft.Text(f"Teilprobe {i}", color="yellow", weight="bold"), c["name"], c["ort"], ft.Text("Herstellungsdatum:", color="white"), ft.Row([c["h_t"], c["h_m"], c["h_j"]], wrap=True, alignment=ft.MainAxisAlignment.CENTER), ft.Text("Verbrauchsdatum:", color="white"), ft.Row([c["v_t"], c["v_m"], c["v_j"]], wrap=True, alignment=ft.MainAxisAlignment.CENTER), c["inhalt"], c["verpackung"], c["temp"], ft.Divider(color="white24")])
                    elif sub == "okz":
                ...
                haupt_bereich.controls.extend([
                    ...
                    ft.Row([action_btn("Laden", lade_v, "#2196F3"), action_btn("🗑️", del_v, "#F44336")], alignment=ft.MainAxisAlignment.CENTER), 
                    ft.Divider(color="white24"), vl_name_in, 
                    action_btn("Als Vorlage speichern", save_v, "#FF9800")
                ])
            page.update()
        ...
        def hole_aktuelle_daten():
            d = {
                "datum": get_date_str(tag_dd.value, mon_dd.value, jahr_dd.value), "adresse": adr_in.value, "marktnummer": nr_in.value, "auftragsnummer": auft_in.value, "mitarbeiter_name": name_in.value, "auftraggeber": ag_dd.value, "typ_probenahme": typ_dd.value, "bemerkung": bem_in.value,
                "tw_kalt": tw_kalt_cb.value, "tw_lims_override": lims_override_cb.value, "tw_zeit": tw_zeit_in.value, "tw_temp": tw_temp_in.value, "tw_desinf": tw_desinf_dd.value, "tw_zapf": tw_zapf_dd.value, "tw_cb_pn": cb_pn.value, "tw_cb_zwei": cb_zwei.value, "tw_cb_sensor": cb_sensor.value, "tw_cb_knie": cb_knie.value, "tw_cb_ein": cb_ein.value, "tw_cb_ein_g": cb_ein_g.value, "tw_cb_eck": cb_eck.value, "tw_zapf_sonst": tw_zapf_sonst_dd.value, "tw_inaktiv": tw_inaktiv_dd.value, "tw_kurz1": tw_kurz1_dd.value, "tw_kurz2": tw_kurz2_dd.value, "tw_kurz3": tw_kurz3_dd.value, "tw_kurz4": tw_kurz4_dd.value, "tw_auff_ja": cb_auff_ja.value, "tw_auff_nein": cb_auff_nein.value, "tw_auff_perlator": cb_auff_perl.value, "tw_auff_kalk": cb_auff_verkalk.value, "tw_auff_verbrueh": cb_auff_verbrueh.value, "tw_auff_durchlauf": cb_auff_durchlauf.value, "tw_auff_eckventil": cb_auff_eck_zu.value, "tw_auff_unterbau": cb_auff_unterbau.value, "tw_auff_unmoeglich": cb_auff_nichtmoeglich.value, "tw_auff_dusche": cb_auff_dusche.value, "tw_auff_handbrause": cb_auff_handbrause.value, "tw_auff_sonstiges": cb_auff_sonst.value, "tw_auff_sonst_text": tw_auff_sonstiges_in.value, "tw_zweck": tw_zweck_dd.value, "tw_inhalt": tw_inhalt_in.value, "tw_verpackung": tw_verpackung_dd.value, "tw_entnahmeort": tw_entnahmeort_dd.value, "tw_tempkonst": tw_tempkonst_in.value, "tw_bemerkung_2": tw_bemerkung_dd.value,
                "se_kalt": se_kalt_cb.value, "se_zeit": se_zeit_in.value, "se_zapf": se_zapf_dd.value, "se_cb_eiswanne": se_cb_eiswanne.value, "se_cb_fallprobe": se_cb_fallprobe.value, "se_tech_sonst": se_tech_sonst_in.value, "se_desinf": se_desinf_dd.value, "se_cb_ozon": se_cb_ozon.value, "se_auff_sonst": se_auff_sonst_in.value, "se_inhalt": se_inhalt_in.value, "se_verpackung": se_verpackung_dd.value, "se_entnahmeort": se_entnahmeort_dd.value, "se_temp": se_temp_in.value, "se_bemerkung": se_bemerkung_dd.value, "se_abklatsch_cb": se_okz_cb.value, "se_abklatsch_bemerkung": se_okz_bemerkung_dd.value,
                ...
                "hfm_bio_cb": hfm_bio_cb.value, "hfm_bio_entnahmeort": hfm_bio_entnahmeort_dd.value, "hfm_bio_inhalt": hfm_bio_inhalt_in.value, "hfm_bio_verpackung": hfm_bio_verpackung_dd.value, "hfm_bio_lief_schwein": hfm_bio_lief_schwein_in.value, "hfm_bio_lief_rind": hfm_bio_lief_rind_in.value, "hfm_bio_mhd_schwein": get_date_str(hfm_bio_mhd_s_tag_dd.value, hfm_bio_mhd_s_mon_dd.value, hfm_bio_mhd_s_jahr_dd.value), "hfm_bio_mhd_rind": get_date_str(hfm_bio_mhd_r_tag_dd.value, hfm_bio_mhd_r_mon_dd.value, hfm_bio_mhd_r_jahr_dd.value), "hfm_bio_charge_schwein": hfm_bio_charge_schwein_dd.value, "hfm_bio_charge_rind": hfm_bio_charge_rind_dd.value, "hfm_bio_temp": hfm_bio_temp_in.value, "hfm_bio_bemerkung": hfm_bio_bemerkung_dd.value, "hfm_abklatsch_cb": hfm_okz_cb.value, "hfm_abklatsch_bemerkung": hfm_okz_bemerkung_dd.value,
                "og_cb": og_cb.value, "og_abklatsch_cb": og_okz_cb.value, "og_abklatsch_bemerkung_1": og_okz_bemerkung_dd.value, "og_abklatsch_bemerkung_2": og_okz_anmerkung_in.value
            }
            for idx, c in se_okz_controls.items(): d[f"0003_status_{idx}"] = c["status"].value; d[f"0003_objekt_{idx}"] = c["objekt"].value; d[f"0003_ort_{idx}"] = c["ort"].value; d[f"0003_abklatsch_{idx}"] = c["abklatsch"].value; d[f"0003_tupfer_{idx}"] = c["tupfer"].value
            for idx, c in okz_controls.items(): d[f"0010_status_{idx}"] = c["status"].value; d[f"0010_objekt_{idx}"] = c["objekt"].value; d[f"0010_ort_{idx}"] = c["ort"].value; d[f"0010_abklatsch_{idx}"] = c["abklatsch"].value; d[f"0010_tupfer_{idx}"] = c["tupfer"].value
            for i in range(1, 6):
            ...
            for idx, c in og_okz_controls.items(): d[f"0011_status_{idx}"] = c["status"].value; d[f"0011_objekt_{idx}"] = c["objekt"].value; d[f"0011_ort_{idx}"] = c["ort"].value; d[f"0011_abklatsch_{idx}"] = c["abklatsch"].value; d[f"0011_tupfer_{idx}"] = c["tupfer"].value
            return d
        ...
        ansicht.controls.extend([
            ...
            # DEINE WUNSCH-BUTTONS: Dunkler Hintergrund, fetter Rand in Signalfarben!
            ft.Row([
                action_btn("🚚 Zurück", lambda e: zeige_dashboard(), "#F44336"),
                action_btn("💾 Speichern", nur_speichern, "#FF9800"),
                action_btn("📄 Bericht (PDF)", save_final, "#2196F3")
            ], wrap=True, alignment=ft.MainAxisAlignment.CENTER)
        ])
        
        switch_tab("stamm") # Startet direkt mit Stammdaten
