...
def zeige_maske_ui(page: ft.Page, ansicht: ft.Column, nav_leiste, zeige_dashboard, zeige_fehler, markt_index):
    try:
        ...
        # --- 5. CONVENIENCE (umbenannt in "Proben") ---
        ...
        def switch_tab(tab_id):
            ...
            # Umbenennung: Convenience -> Proben
            ...
            if tab_id == "stamm":
                ...
                # FIX: wrap=True auch hier, damit die Unter-Tabs nicht quetschen
                ...
                # FIX: wrap=True und CENTER Ausrichtung
                sub_nav = ft.Row(wrap=True, alignment=ft.MainAxisAlignment.CENTER)
                sub_cont = ft.Column()
                ...
                def sw_hfm(sub):
                    sub_cont.controls.clear(); sub_nav.controls.clear()
                    # Die Liste der Unter-Tabs
                    tabs_hfm = [("hack", "🥩 Hack"), ("mett", "🍖 Mett"), ("fzs", "🐷 FZ Schwein"), ("fzg", "🐔 FZ Geflügel"), ("bio", "🥩 Bio"), ("okz", "🔬 OKZ")]
                    
                    for sid, sname in tabs_hfm:
                        is_active = (sid == sub)
                        bg = "#004400" if is_active else "#1a1a1a"
                        
                        # FIX: padding=8 macht die Buttons viel flacher!
                        btn = ft.ElevatedButton(
                            sname, on_click=lambda e, s=sid: sw_hfm(s), 
                            bgcolor=bg, color="white", 
                            style=ft.ButtonStyle(
                                shape=ft.RoundedRectangleBorder(radius=10), padding=8, 
                                side=ft.BorderSide(width=1.5, color="#4CAF50") # Dünner Neon-Schein
                            )
                        )
                        sub_nav.controls.append(btn)
                ...
                def sw_og(sub):
                    sub_cont.controls.clear(); sub_nav.controls.clear()
                    # Umbenennung auch hier: "🥗 Convenience" -> "🥗 Proben"
                    tabs_og = [("teil", "🥗 Proben"), ("okz", "🔬 OKZ")]
                    
                    for sid, sname in tabs_og:
                        is_active = (sid == sub)
                        bg = "#004400" if is_active else "#1a1a1a"
        ...
        def check_pflichtfelder():
            ...
            if not (auft_in.value or "").strip(): errors.append("- Auftragsnummer / Etikettenummer")
